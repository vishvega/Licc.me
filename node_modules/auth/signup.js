            ...
            if (check.username(username) && check.tag(tag)) {
                if (check.email(email)) {
                    if (password.length >= 8 && password === verify) {
                        ...
                        const count = await db.collection("users").countDocuments({ username: username, tag: tag }, { limit: 1 });

                        if (count === 0) {
                            const unique = await db.collection("users").countDocuments({ email: email }, { limit: 1 });
