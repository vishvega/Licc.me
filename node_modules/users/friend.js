        ...
        if (typeof message.username === "string") {
            ...
            if (split.length === 2) {
                ...
                if (check.username(username) && check.tag(tag)) {
                    ...
                    if (find !== null) {
                        const user_id = find._id.toString(), avatarURL = image.url(find.avatar, "avatars");

                        if (session.user_id !== user_id) {
                            const count = await db.collection("dm_channels").countDocuments({ $or: [ { $and: [ { client: ObjectId(session.user_id) }, { recipient: ObjectId(user_id) } ] }, { $and: [ { client: ObjectId(user_id) }, { recipient: ObjectId(session.user_id) } ] } ] }, { limit: 1 });
