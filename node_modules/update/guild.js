...
module.exports = async (session, query, message) => {
    if (await session.set()) {
        ...
        if (await channel.isOwner(guild_id, session.user_id)) {
            ...
            if (find !== null) {
                const { icon } = find;
                const change = channel.isValidName(name) ? name : find.name;

                const filename = await image.save(file, "icons", 256);
                const result = await db.collection("guilds").findOneAndUpdate({ _id: ObjectId(guild_id) }, { $set: { name: change, icon: filename === null ? icon : filename } }, { projection: { _id: 0, name: 1, icon: 1, channels: 1 } });

                if (result.value !== null) {
                    if (filename !== null) {
                        if (icon !== null) {
                            try {
                                fs.unlinkSync(`public/icons/${icon}`);
                            } catch (error) {
                    ...
                    return {
                        ...
                        guild: {
                            id: guild_id,
                            name: change,
                            icon: image.url(filename !== null ? filename : icon, "icons")
                        }
                    }
                } else {
                    return {
