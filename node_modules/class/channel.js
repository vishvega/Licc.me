...
class Channel {
    ...
    static async isOwner(guild_id, user_id) {
        if (this.isValidId(guild_id)) {
            ...
            const count = await db.collection("guilds").countDocuments({ _id: ObjectId(guild_id), owner: ObjectId(user_id) });
    ...
    static async hasDMPermission(channel_id, user_id) {
        if (channel_id === this.EVERYONE) {
            ...
            const db = await require("class/database.js");
            const count = await db.collection("dm_channels").countDocuments({ _id: ObjectId(channel_id), $or: [{ client: ObjectId(user_id) }, { recipient: ObjectId(user_id) }] }, { limit: 1 });
